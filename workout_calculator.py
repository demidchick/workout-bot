def calculate_next_workout(name, current_weight, current_reps, sets, weight_increment):
    ...
    def sort_key(combo):
        ...
        # Prioritize: closest to 2% increase (but not below), closest to current weight, closest to current reps
        return (
            abs(2 - increase) if increase >= 2 else float('inf'),  # Forces increases below 2% to be last choice
